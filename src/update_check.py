...
def launch_installer() -> bool:
    ...
    On Windows the installer is run with /SILENT.  The installer's [Run]
    section (postinstall, no skipifsilent) relaunches the app automatically
    once the install completes.  The caller should call ``pygame.quit()``
    and exit so the installer can replace the running files.
    """
    with _dl_lock:
    ...
    try:
        if sys.platform == "win32":
            # /SILENT = no wizard UI, /NORESTART = don't reboot PC.
            # The installer's [Run] section (postinstall, no skipifsilent)
            # relaunches the app automatically — no separate restart helper needed.
            subprocess.Popen(
